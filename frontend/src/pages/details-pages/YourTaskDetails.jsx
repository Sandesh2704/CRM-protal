...
export default function YourTaskDetails({ fetchTasks }) {
    ...
    const [task, setTask] = useState(location.state?.assignTaskDetails || "nothhing");
    ...
    const handleUpdateSubmit = async () => {
        ...
        try {
            ...
            if (response.status === 200) { // Check for success status
                ...
                // Handle unexpected successful but invalid responses
                toast.error("Unexpected response while updating the task.");
            }
        } catch (error) {
            // Log the error for debugging
            console.error("Failed to update task:", error);

            // Use a specific error message or fallback
            const errorMessage =
...
console.log("assignerId ", assignerId, "deadline", deadline, title, description, updates, status, documents )
    return (
        <>
